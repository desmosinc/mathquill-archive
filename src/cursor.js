...
var Cursor = P(function(_) {
  ...
  _.seekHoriz = function(pageX, block) {
    ...
    while (dist > 0 && (ghost.prev || ghost.parent !== block)) {
      //FIXME HACK
      //with x_1^2, when the ghost cursor is in the exponent,
      ...
      //into the subscript, the ghost cursor should go to the "top" block
      ...
      if (!ghost.prev && ghost.parent.parent.respaced) {
        ghost = appendGhostTo(ghost.parent.parent.prev.firstChild);
      }
      else {
