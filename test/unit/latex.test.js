suite('latex', function() {
  ...
  suite('\\MathQuillMathField', function() {
    ...
    test('optional classnames', function() {
      var outer = $('<span>y=\\MathQuillMathField[amplitude]{A}\\sin\\left(\\frac{2\\pi x}{\\MathQuillMathField[wavelength denominator]{\\lambda}}\\right)</span>')
        .appendTo('#mock').mathquill();

      var mathFields = outer.find('.mathquill-editable');
      assert.ok(mathFields.first().hasClass('amplitude'));
      assert.ok(mathFields.last().hasClass('wavelength'));
      assert.ok(mathFields.last().hasClass('denominator'));
